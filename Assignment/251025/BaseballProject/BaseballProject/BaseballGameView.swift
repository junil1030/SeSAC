...
extension BaseballGameView {
    ...
    private func generateSecretNumber() -> [Int] {
        let numbers = Array(0...9).shuffled().prefix(3)
...
//        return [1,2,3]
    ...
    private func validateInput(_ input: String) -> (isValid: Bool, message: String) {
        guard let validNum = Int(input) else { return (false, "숫자를 입력해주세요.") }
        print(validNum)
        if validNum < 0 || validNum > 1000 {
            return (false, "세 자리 숫자를 입력해주세요.")
        }
        return (true, "")
    }
    ...
    private func checkGuess() {
        ...
        let userNum = Array(userInput).map { Int(String($0))! }
        for i in 0..<secretNumber.count {
            if secretNumber[i] == userNum[i] {
                strikes += 1
            } else if secretNumber.contains(userNum[i]) {
                balls += 1
            }
        }
