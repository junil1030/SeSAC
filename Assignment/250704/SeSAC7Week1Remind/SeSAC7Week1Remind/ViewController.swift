...
class ViewController: UIViewController {
    ...
    func getZodiacSign(month: Int) {
        ...
        resultLabel.text = "\(month)월은 \(star)자리입니다."
    }
    ...
    func getRecommandGameJob(strength: Int, agility: Int) {
        ...
        print("힘: \(strength), 민첩: \(agility)")
        resultLabel.text = "\(stat)이 가장 높으니 \(job)를 추천드립니다."
    }
    ...
    func getRecommandClothing(temperature: Int, isRaining: Bool) {
        ...
        var umbrellaAdvice = "비가 오지 않으니 우산은 안챙겨도 됩니다"
        
        if isRaining {
            umbrellaAdvice = "비가 오니 우산을 챙기세요"
        }
        
        if isSnow {
            umbrellaAdvice = umbrellaAdvice.replacingOccurrences(of: "비가", with: "눈이")
        }
        
        resultLabel.text = """
                            지금 온도가 \(temperature)도 이니 \(advice). 
                            \(umbrellaAdvice).  
                            """
    }
    ...
    @IBAction func evaluateGrade(_ sender: UIButton) {
        ...
        let all = data.count
        let avarage = Double(data.reduce(0, +)) / Double(all)
        
        resultLabel.text = "\(all)명의 평균 점수는 \(round(avarage * pow(10, 3)) / pow(10, 3))점입니다."
    }
